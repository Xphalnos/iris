...
void ata_isif_read_sector(void* udata, uint64_t lba, uint8_t* buf) {
    struct isif_state* isif = (struct isif_state*)udata;

    isif_read_block(isif, lba, buf);
}

void ata_isif_write_sector(void* udata, uint64_t lba, const uint8_t* buf) {
    struct isif_state* isif = (struct isif_state*)udata;

    isif_write_block(isif, lba, buf);
}

int ata_isif_get_identify(void* udata, uint8_t* buf) {
    struct isif_state* isif = (struct isif_state*)udata;

    if (isif->hdr.extension_offset) {
        isif_read_extension(isif, buf);
...
uint64_t ata_isif_get_sector_count(void* udata) {
    struct isif_state* isif = (struct isif_state*)udata;

    return isif->hdr.block_count;
}

void ata_isif_close(void* udata) {
    struct isif_state* isif = (struct isif_state*)udata;
